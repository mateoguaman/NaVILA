...
def merge_predictions(save_f: str, filenames: List[str], enforce_unique: bool = True) -> None:
    ...
    if filenames[0].endswith(".jsonl"):  # merging rxr predictions
        ...
        for fn in filenames:
            with open(fn) as f:
                ...
                overlapping_keys = set(merged_predictions.keys()) & set(new_predictions.keys())
                assert (
